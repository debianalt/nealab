    ...
    # ── VIVIR ─────────────────────────────────────────────────────────────
    {
        ...
        "sql": """
            SELECT r.redcode,
                ...
                COALESCE(t.ruggedness_mean, 0) AS c_ruggedness,
                COALESCE(t.twi_mean, 0) AS c_twi,
                COALESCE(c.days_gt_50mm, 0) AS c_rain_extreme
            ...
            LEFT JOIN (
                SELECT redcode, AVG(days_gt_50mm) AS days_gt_50mm
                FROM chirps_annual WHERE year >= 2019 GROUP BY redcode
            ) c ON r.redcode = c.redcode
        """,
        "components": [
            ("c_fire", "c_fire", 0.20, False),
            ("c_deforest", "c_deforest", 0.20, False),
            ("c_thermal_amp", "c_thermal_amp", 0.15, False),
            ("c_slope", "c_slope", 0.10, False),
            ("c_hand", "c_hand", 0.10, True),
            ("c_ruggedness", "c_ruggedness", 0.10, False),
            ("c_twi", "c_twi", 0.05, True),
            ("c_rain_extreme", "c_rain_extreme", 0.10, False),
        ],
    },
    {
        "id": "climate_comfort",
        "sql": """
            SELECT r.redcode,
                COALESCE(e.temp_mean, 20) AS c_mean_temp,
                ...
                COALESCE(t.elev_mean, 200) AS c_elevation,
                COALESCE(e.dewpoint_mean, 15) AS c_dewpoint,
                COALESCE(e.solar_radiation, 15) AS c_solar,
                COALESCE(e.gdd_base10, 0) AS c_gdd,
                COALESCE(et2.mean_et, 0) AS c_et
            FROM radios_misiones r
            LEFT JOIN (
                SELECT redcode, AVG(mean_lst_night) AS mean_lst_night
                FROM lst_annual WHERE year >= 2019 GROUP BY redcode
            ) l ON r.redcode = l.redcode
            LEFT JOIN (
            ...
            LEFT JOIN (
                ...
                FROM et_annual WHERE year >= 2019 GROUP BY redcode
            ) et2 ON r.redcode = et2.redcode
        """,
        "components": [
            ("c_mean_temp", "c_mean_temp", 0.10, True),
            ("c_heat_night", "c_heat_night", 0.10, True),
            ("c_precipitation", "c_precipitation", 0.10, False),
            ("c_frost", "c_frost", 0.10, True),
            ("c_elevation", "c_elevation", 0.10, True),
            ("c_dewpoint", "c_dewpoint", 0.10, False),
            ("c_solar", "c_solar", 0.10, False),
            ("c_gdd", "c_gdd", 0.10, False),
            ("c_et", "c_et", 0.10, False),
        ],
    },
    {
        "id": "green_capital",
    ...
    # ── INVERTIR ──────────────────────────────────────────────────────────
    {
        ...
        "sql": """
            SELECT r.redcode,
                ...
                COALESCE(g2.built_fraction, 0) AS c_built_2020,
                COALESCE(h.treecover2000, 0) AS c_treecover_base,
                COALESCE(ce.densidad_hab_km2, 0) AS c_pop_density,
                COALESCE(v_level.mean_radiance, 0) AS c_viirs_level,
            ...
            ) v_level ON r.redcode = v_level.redcode
            LEFT JOIN road_access rd ON r.redcode = rd.redcode
        """,
        "components": [
            ("c_viirs_trend", "c_viirs_trend", 0.10, False),
            ("c_ghsl_change", "c_ghsl_change", 0.10, False),
            ("c_hansen_loss", "c_hansen_loss", 0.10, False),
            ("c_ndvi_trend", "c_ndvi_trend", 0.10, True),
            ("c_fire_count", "c_fire_count", 0.10, False),
            ("c_built_2020", "c_built_2020", 0.10, False),
            ("c_treecover_base", "c_treecover_base", 0.10, False),
            ("c_pop_density", "c_pop_density", 0.05, False),
            ("c_viirs_level", "c_viirs_level", 0.10, False),
            ("c_road_density", "c_road_density", 0.15, False),
        ],
    },
    {
        "id": "location_value",
        "sql": """
            SELECT r.redcode,
                ...
                COALESCE(g.built_fraction, 0) AS c_built_fraction
            FROM radios_misiones r
        ...
        """,
        "components": [
            ("c_access_20k", "c_access_20k", 0.15, True),
            ("c_healthcare", "c_healthcare", 0.15, True),
            ("c_nightlights", "c_nightlights", 0.15, False),
            ("c_slope", "c_slope", 0.10, True),
            ("c_road_dist", "c_road_dist", 0.10, True),
            ("c_pop_density", "c_pop_density", 0.10, False),
            ("c_road_density", "c_road_density", 0.15, False),
            ("c_access_50k", "c_access_50k", 0.10, True),
            ("c_built_fraction", "c_built_fraction", 0.10, False),
        ],
    },
    # ── PRODUCIR ──────────────────────────────────────────────────────────
    {
        ...
        "sql": """
            SELECT r.redcode,
                ...
                COALESCE(e.gdd_base10, 0) AS c_gdd,
                COALESCE(t.slope_mean, 0) AS c_slope,
                COALESCE(s.nitrogen, 0) AS c_nitrogen,
                COALESCE(t.elev_mean, 200) AS c_elevation,
                COALESCE(s.cec, 0) AS c_cec
            FROM radios_misiones r
        ...
        """,
        "components": [
            ("c_soc", "c_soc", 0.15, False),
            ("c_clay", "c_clay", 0.10, False),
            ("c_precipitation", "c_precipitation", 0.15, False),
            ("c_gdd", "c_gdd", 0.10, False),
            ("c_slope", "c_slope", 0.10, True),
            ("c_nitrogen", "c_nitrogen", 0.15, False),
            ("c_elevation", "c_elevation", 0.10, True),
            ("c_cec", "c_cec", 0.15, False),
        ],
    },
    {
        "id": "forest_health",
        "sql": """
            SELECT r.redcode,
                ...
                COALESCE(h.treecover2000, 0) AS c_treecover,
                COALESCE(la.mean_lai, 0) AS c_lai,
                COALESCE(n.mean_ndvi, 0) AS c_ndvi_level,
                COALESCE(np.mean_npp, 0) AS c_npp
            FROM radios_misiones r
            LEFT JOIN (
                SELECT redcode, regr_slope(mean_ndvi, year) AS ndvi_slope
        ...
        """,
        "components": [
            ("c_ndvi_trend", "c_ndvi_trend", 0.10, False),
            ("c_loss_ratio", "c_loss_ratio", 0.10, True),
            ("c_fire", "c_fire", 0.10, True),
            ("c_gpp", "c_gpp", 0.10, False),
            ("c_et", "c_et", 0.10, False),
            ("c_treecover", "c_treecover", 0.10, False),
            ("c_lai", "c_lai", 0.10, False),
            ("c_ndvi_level", "c_ndvi_level", 0.15, False),
            ("c_npp", "c_npp", 0.15, False),
        ],
    },
    {
        ...
        """,
        "components": [
            ("c_clay", "c_clay", 0.10, True),
            ("c_precipitation", "c_precipitation", 0.15, False),
            ("c_slope", "c_slope", 0.15, True),
            ("c_access_50k", "c_access_50k", 0.15, True),
            ("c_soc", "c_soc", 0.15, False),
            ("c_elevation", "c_elevation", 0.15, True),
    ...
    # ── SERVIR ────────────────────────────────────────────────────────────
    {
        ...
        "sql": """
            SELECT r.redcode,
                ...
                COALESCE(ce.pct_sin_techo_adecuado, 0) AS c_sin_techo,
                COALESCE(ce.pct_jefatura_femenina, 0) AS c_jefatura_fem
            FROM radios_misiones r
            LEFT JOIN censo2022_variables ce ON r.redcode = ce.redcode
        """,
        "components": [
            ("c_nbi", "c_nbi", 0.15, False),
            ("c_sin_cloacas", "c_sin_cloacas", 0.10, False),
            ("c_piso", "c_piso", 0.15, False),
            ("c_hacinamiento", "c_hacinamiento", 0.15, False),
            ("c_hacinamiento_crit", "c_hacinamiento_crit", 0.05, False),
            ("c_combustible", "c_combustible", 0.15, False),
            ("c_sin_techo", "c_sin_techo", 0.15, False),
        ],
    },
    {
    ...
    },
    {
        ...
        "sql": """
            SELECT r.redcode,
                ...
                100.0 - COALESCE(ce.pct_computadora, 0) AS c_sin_computadora,
                COALESCE(o.accessibility_cities_min, 300) AS c_access_cities,
                COALESCE(rd.dist_primary_m, 50000) AS c_dist_primary
            FROM radios_misiones r
            LEFT JOIN oxford_accessibility o ON r.redcode = o.redcode
            LEFT JOIN censo2022_variables ce ON r.redcode = ce.redcode
            LEFT JOIN road_access rd ON r.redcode = rd.redcode
        ...
        """,
        "components": [
            ("c_healthcare_time", "c_healthcare_time", 0.15, False),
            ("c_health_coverage", "c_health_coverage", 0.10, True),
            ("c_nbi", "c_nbi", 0.15, False),
            ("c_elderly", "c_elderly", 0.10, False),
            ("c_children", "c_children", 0.10, False),
            ("c_road_density", "c_road_density", 0.10, True),
            ("c_nightlights", "c_nightlights", 0.15, True),
            ("c_sin_computadora", "c_sin_computadora", 0.10, False),
            ("c_access_cities", "c_access_cities", 0.10, False),
            ("c_dist_primary", "c_dist_primary", 0.10, False),
        ],
    },
    {
        ...
        "sql": """
            SELECT r.redcode,
                ...
                COALESCE(ce.pct_solo_primaria, 0) AS c_solo_primaria,
                COALESCE(ce.tasa_asistencia_13a17, 0) AS c_asistencia_sec,
                COALESCE(v.mean_radiance, 0) AS c_nightlights,
                COALESCE(ce.pct_cobertura_salud, 0) AS c_coverage_salud
            FROM radios_misiones r
            LEFT JOIN censo2022_variables ce ON r.redcode = ce.redcode
            LEFT JOIN (
                SELECT redcode, AVG(mean_radiance) AS mean_radiance
        ...
        """,
        "components": [
            ("c_no_schooling", "c_no_schooling", 0.15, False),
            ("c_secondary_plus", "c_secondary_plus", 0.15, True),
            ("c_higher_edu", "c_higher_edu", 0.15, True),
            ("c_university", "c_university", 0.10, True),
            ("c_solo_primaria", "c_solo_primaria", 0.15, False),
            ("c_asistencia_sec", "c_asistencia_sec", 0.10, True),
            ("c_nightlights", "c_nightlights", 0.10, True),
            ("c_coverage_salud", "c_coverage_salud", 0.10, True),
        ],
    },
    {
        "id": "education_flow",
        "sql": """
            SELECT r.redcode,
                ...
                COALESCE(ce.pct_solo_primaria, 0) AS c_solo_primaria,
                COALESCE(ce.pct_jefatura_femenina, 0) AS c_jefatura_fem,
                COALESCE(ce.indice_dependencia, 0) AS c_dependencia,
                COALESCE(ce.pct_monoparental, 0) AS c_monoparental
            FROM radios_misiones r
            LEFT JOIN censo2022_variables ce ON r.redcode = ce.redcode
        """,
        "components": [
            ("c_dropout_primary", "c_dropout_primary", 0.20, False),
            ("c_dropout_secondary", "c_dropout_secondary", 0.20, False),
            ("c_teen_pregnancy", "c_teen_pregnancy", 0.15, False),
            ("c_nbi", "c_nbi", 0.15, False),
            ("c_solo_primaria", "c_solo_primaria", 0.15, False),
            ("c_dependencia", "c_dependencia", 0.10, False),
        ],
    },
]
