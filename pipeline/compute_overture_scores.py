...
def compute_scores(conn: duckdb.DuckDBPyConnection, output_path: str, emit_diagnostics: bool = False) -> int:
    ...
    conn.execute(f"""
        ...
        SELECT
            ...
            CASE WHEN COALESCE(t.n_primary, 0) + COALESCE(t.n_secondary, 0) + COALESCE(t.n_tertiary, 0) > 0
                 THEN 1.0 ELSE 0.0
            END AS uc_hierarchy,
            LEAST(1.0, (COALESCE(ba.n_power, 0) + COALESCE(ba.n_water_infra, 0) + COALESCE(ba.n_communication, 0))::FLOAT / 5.0) AS uc_infra,
            LEAST(1.0, COALESCE(b.avg_floors, 0)::FLOAT / 3.0) AS uc_height,
    ...
    conn.execute(f"""
        ...
        SELECT h.h3index,
            ...
            CASE WHEN COALESCE(t.n_paved, 0) + COALESCE(t.n_unpaved, 0) + COALESCE(t.n_track, 0) = 0 THEN 0
                 ELSE COALESCE(t.n_paved, 0)::FLOAT / (COALESCE(t.n_paved, 0) + COALESCE(t.n_unpaved, 0) + COALESCE(t.n_track, 0))
            END AS rc_paving,
            CASE WHEN COALESCE(t.segment_count, 0) = 0 THEN 0
                 ELSE LEAST(1.0, (COALESCE(t.n_pedestrian, 0) + COALESCE(t.n_footway, 0) + COALESCE(t.n_cycleway, 0) + COALESCE(t.n_living_street, 0))::FLOAT / t.segment_count)
            END AS rc_pedestrian,
            LEAST(1.0, (COALESCE(t.n_primary, 0) + COALESCE(t.n_secondary, 0))::FLOAT / GREATEST(1, t.segment_count)) AS rc_major_roads,
    ...
    conn.execute(f"""
        ...
        SELECT h.h3index,
            ...
            LEAST(1.0, COALESCE(ba.infra_count, 0)::FLOAT / 5.0) AS urb_infra,
            CASE WHEN COALESCE(t.n_paved, 0) + COALESCE(t.n_unpaved, 0) + COALESCE(t.n_track, 0) = 0 THEN 0
                 ELSE COALESCE(t.n_paved, 0)::FLOAT / (COALESCE(t.n_paved, 0) + COALESCE(t.n_unpaved, 0) + COALESCE(t.n_track, 0))
            END AS urb_paving,
            CASE WHEN COALESCE(b.building_count, 0) = 0 THEN 0
                 ELSE LEAST(1.0, COALESCE(b.n_commercial, 0)::FLOAT / b.building_count)
            END AS urb_commercial,
    ...
    conn.execute(f"""
        COPY (
            SELECT
                ...
                COALESCE(su.uc_infra, 0)          AS uc_infra,
                COALESCE(su.uc_height, 0)         AS uc_height,
                COALESCE(sr.rc_hierarchy, 0)       AS rc_hierarchy,
                COALESCE(sr.rc_density, 0)         AS rc_density,
                COALESCE(sr.rc_bridges, 0)         AS rc_bridges,
                COALESCE(sr.rc_paving, 0)          AS rc_paving,
                COALESCE(sr.rc_pedestrian, 0)      AS rc_pedestrian,
                COALESCE(sr.rc_major_roads, 0)     AS rc_major_roads,
                ...
                COALESCE(surb.urb_infra, 0)       AS urb_infra,
                COALESCE(surb.urb_paving, 0)      AS urb_paving,
                COALESCE(surb.urb_commercial, 0)  AS urb_commercial,
    ...
    composites = {
        "urban_consolidation": ["uc_density", "uc_height", "paving_index", "commercial_vitality", "building_mix", "service_access"],
        "road_connectivity": ["rc_hierarchy", "rc_density", "rc_paving", "rc_pedestrian", "paving_index", "commercial_vitality", "building_mix"],
        "urbanization": ["urb_density", "urb_paving", "urb_infra", "paving_index", "service_access", "commercial_vitality", "building_mix"],
    }
    ...
    for score_col, raw_cols in composites.items():
        ...
        if emit_diagnostics:
            generate_report(score_col, diagnostics, output_dir=os.path.dirname(output_path))
...
def main():
    ...
    parser.add_argument("--diagnostics", action="store_true", help="Emit *_diagnostics.json for each composite score")
    ...
    row_count = compute_scores(conn, args.output, emit_diagnostics=args.diagnostics)
    conn.close()
