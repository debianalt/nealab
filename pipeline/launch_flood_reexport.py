...
def main() -> int:
    ...
    for tid in TERRITORIES:
        print(f"--- {tid} ---")
        try:
            # historical=False: el S1 recurrence mensual (2015→hoy, ~4h/territorio)
            # NO se usa en el score (este usa la banda JRC jrc_recurrence). Omitirlo.
            tasks = launch_exports(territory_id=tid, historical=False,
