<script lang="ts">
	...
	// Cobertura 2020 (MapBiomas) de las celdas con pérdida arbórea en una unidad EUDR.
	...
	let eudrUnitStats: { plantation: number; native: number; other: number; conversion: number; dataCells: number } | null = $state(null);
	...
	async function computeEudrUnitPlantation(cells: string[]) {
		...
		try {
			...
			let plantation = 0, native = 0, other = 0, conversion = 0, dataCells = 0;
			const T = 40;
			for (const r of rows) {
				if (r.pn != null || r.p20 != null || r.n20 != null) dataCells++;
				if (Number(r.def) > 0) {
			...
			eudrUnitStats = { plantation, native, other, conversion, dataCells };
	...
	// PY districts have no vector-tile geometry usable for export (tiles clip
	// the large polygons into seams). Their full unclipped polygons ship as a
	// static asset generated by pipeline/build_itapua_districts.py --export-geojson.
	const districtGeoCache = new Map<string, any>();
	async function downloadDistrictGeoJson(
		distrito: string,
		territory: string,
		properties: Record<string, any>
	): Promise<void> {
		const stem = territory === 'alto_parana_py' ? 'alto_parana' : 'itapua';
		let fc = districtGeoCache.get(stem);
		if (!fc) {
			try {
				const res = await fetch(`/data/${stem}_districts.geojson`);
				if (!res.ok) throw new Error(`HTTP ${res.status}`);
				fc = await res.json();
				districtGeoCache.set(stem, fc);
			} catch (e) {
				console.warn('District polygons fetch failed for', stem, e);
				return;
			}
		}
		const feature = fc.features?.find((f: any) => f.properties?.district === distrito);
		if (!feature) {
			console.warn('District geometry not available for', distrito, `(${stem})`);
			return;
		}
		const out = {
			type: 'FeatureCollection',
			features: [{ type: 'Feature', geometry: feature.geometry, properties: { distrito, ...properties } }]
		};
		const safe = distrito.replace(/[^\p{L}\p{N}]+/gu, '_').replace(/^_+|_+$/g, '') || 'distrito';
		const blob = new Blob([JSON.stringify(out)], { type: 'application/geo+json' });
		const url = URL.createObjectURL(blob);
		const a = document.createElement('a');
		a.href = url;
		a.download = `spatia_distrito_${safe}.geojson`;
		document.body.appendChild(a);
		a.click();
		document.body.removeChild(a);
		URL.revokeObjectURL(url);
			...
			{#if lensStore.activeAnalysis?.id === 'eudr' && eudrUnit}
				<div class="absolute top-3 left-1/2 -translate-x-1/2 z-30 px-4 py-2 rounded-2xl bg-black/80 backdrop-blur-md border border-yellow-400/40 text-[12px] shadow-lg max-w-[400px]">
					...
					{#if eudrUnitStats}
						...
						{:else if (eudrUnitStats.plantation + eudrUnitStats.native + eudrUnitStats.other) === 0}
						...
						{:else}
							{@const total = eudrUnitStats.plantation + eudrUnitStats.native + eudrUnitStats.other}
							...
							<div class="mt-2 pt-2 border-t border-white/10 leading-relaxed">
								...
								<div class="text-[9px] text-white/30 mt-1 leading-snug">{total.toLocaleString()} celdas de ~5 km². Señal indicativa — no determina la causa de la pérdida (cosecha, fuego, conversión) ni constituye veredicto de cumplimiento EUDR.</div>
							</div>
						{/if}
					{/if}
				</div>
