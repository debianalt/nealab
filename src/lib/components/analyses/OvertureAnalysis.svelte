<script lang="ts">
	...
	const ANALYSIS_CONTENT_LEGACY: Record<string, { howToRead: string; implications: string; method: string }> = {
		...
		eudr: {
			...
			method: 'Score compuesto 0-100: 70% pérdida forestal post-2020 (Hansen GFC v1.12, Landsat, remuestreado a 100 m) + 20% área quemada post-2020 (MODIS MCD64A1, 500m) + 10% pérdida de cobertura previa. Cutoff EUDR: 31/12/2020. Resolución: H3 resolución 9 (~0,1 km²); dato satelital subyacente a 100 m, que es el piso de precisión efectivo. Cobertura: 10 provincias del NOA y NEA argentino + 10 departamentos del Paraguay oriental y Chaco + 3 estados del sur de Brasil (Paraná, Santa Catarina, Rio Grande do Sul).',
		},
		accessibility: {
